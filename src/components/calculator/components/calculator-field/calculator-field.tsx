...
export const CalculatorField = ({ fieldName = DEFAULT_FIELD_NAME }: TProps) => {
    const { value, onChange, minFieldValue, maxFiedlValue } =
        useCalculatorField(fieldName);
    ...
    return (
        <>
            <InputNumber
                ...
                max={maxFiedlValue}
                formatter={(value) =>
                    `${value}`.replace(REG_EXP_FOR_FORMAT_MONEY_RANK, ' ')
                }
            />
            <Slider
                onChange={onChange}
                ...
                min={minFieldValue}
                max={maxFiedlValue}
                step={DEFAULT_FIELD_STEP}></Slider>
        </>
    );
};
